...
class SequenceStar(Node):
    ...
    def halt(self):
        print(f"[{self.name}] SequenceStar HALT triggered! Stopping children...")
        if 0 <= self.current_child_index < len(self.children):
            self.children[self.current_child_index].halt()
...
class KeepRunningUntilFailure(Node):
    ...
    def halt(self):
        print(f"[{self.name}] KeepRunningUntilFailure HALT triggered!")
        self.child.halt()
...
class SaveHomePose(Node):
    def __init__(self, node_type, agent, name=None, topic_name="/amcl_pose"):
        ...
        self.saved = False
        self.cache = None
        self.last_cycle_id = None # Added back fix
    ...
    async def run(self, agent, blackboard):
        # 1. Check for New Cycle (Fix for 2nd run crash)
        cycle_id = blackboard.get('alarm_cycle_id')
        if cycle_id != self.last_cycle_id:
            self.saved = False
            self.last_cycle_id = cycle_id
            print(f"[{self.name}] New Cycle Detected. Resetting Home Pose.")
...
class Speak(Node):
    ...
    async def run(self, agent, blackboard):
        # 1. Reset on New Cycle (Fix for repeated runs)
        cycle_id = blackboard.get('alarm_cycle_id')
        if cycle_id != self.last_cycle_id:
            self.played = False
            self.started = False
